...
async def notifications_socket(
    ...
    user = await _authenticate_websocket(websocket, settings)
    if user is None:
    ...
    while True:
        try:
            ...
            prefs = await _handle_client_message(websocket, user.id, raw, prefs)
        except TimeoutError:
            async with AsyncSessionLocal() as session:
                await _send_batch(websocket, session, user.id, prefs)
...
async def _authenticate_websocket(websocket: WebSocket, settings: Settings) -> User | None:
...
async def _handle_client_message(
    websocket: WebSocket, user_id: str, raw: str, prefs: SubscribePayload
) -> SubscribePayload:
    try:
